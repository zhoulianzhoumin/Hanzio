...
void VertMoveObject(TObject *obj) //先移动，再修正
{
	...
	(*obj).vertSpeed += 0.15f; //2.重力加速度，速度每帧增加
	...
	for (int i = 0; i < brickLength; i++) //4.遍历所有砖块，检查碰撞
	{
		if (IsCollision(*obj, brick[i]))
		{
				...
				(obj == &Hanzio)) //比较两个指针是否相等，即是否指向同一个东西
			{
				...
				moving[movingLength - 1].vertSpeed = - 0.7; //GetNewMoving() 刚才把 movingLength 加 1 了，新元素正好在最后一个位置，索引就是 movingLength - 1
			}
			//撞到了：回退Y，停止速度，落地
...
void CreateLevel(int lvl) //重置玩家，分配砖块数组，并把砖块一一摆好
{
	...
	if (lvl == 1)
	{
		...
		InitObject(GetNewBrick(), 60, 15, 10, 10, '#');
			InitObject(GetNewBrick(), 60, 10, 5, 3, '-');
			InitObject(GetNewBrick(), 70, 10, 5, 3, '?');
			InitObject(GetNewBrick(), 75, 10, 5, 3, '-');
			InitObject(GetNewBrick(), 70, 10, 5, 3, '?');
...
int main() 
{
    ...
    // 2. 设置窗口可视区域（相框）
    SMALL_RECT windowRect = { 0, 0, 79, 24 };
	...
	do
	{
		...
		//2.处理输入
		if ((Hanzio.IsFly == FALSE) && (GetAsyncKeyState(VK_SPACE) & 0x8000)) Hanzio.vertSpeed = -0.8f;
