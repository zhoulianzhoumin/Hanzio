...
void VertMoveObject(TObject *obj) //先移动，再修正
{
	...
	(*obj).vertSpeed += 0.08f; //2.重力加速度，速度每帧增加
...
int main() 
{
	...
	do
	{
		...
		//2.处理输入
		if ((Hanzio.IsFly == FALSE) && (GetAsyncKeyState(VK_SPACE) & 0x8000)) Hanzio.vertSpeed = -1.2f;
		...
		setCur(0, mapHeight);
		printf("x=%.1f y=%.1f brick[0].x=%.1f score=%d", Hanzio.x, Hanzio.y, brick[0].x, score);
